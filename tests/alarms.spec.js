...
describe("createAlarms", () => {
  ...
  it("should only create the very next timeSlot alarm for end of restriction", async () => {
      ...
      groups: [
        {
          ...
          restrictions: {
            ...
            consecutiveTime: [
              { days: ["Saturday"], consecutiveTime: 60, pause: 60 },
            ],
          },
        },
      ],
    ...
    expect(global.chrome.alarms.create).toHaveBeenCalledWith(
      "Test-time-slot-restriction-end",
      { delayInMinutes: 60 }
    );
  });

  it("should create the next beginning of time slot restriction on end", async () => {
    let mockGroup = {
      groups: [
        {
          name: "Test",
    ...
    await createAlarms();
    expect(global.chrome.alarms.create).toHaveBeenCalledWith(
      "Test-time-slot-restriction-begin",
      { delayInMinutes: 60 }
    );
  });
  ...
  it("should create an alarm for the beginning of the next restriction period", async () => {
    ...
    // mock sites with or without restrictions
    global.chrome.storage.local.get.mockResolvedValueOnce(mockSites);
    // mocking tabs just for the fun
    global.chrome.tabs.query.mockResolvedValueOnce([])
    await handleOnAlarm({name : "Test-time-slot-restriction-end"});
    expect(global.chrome.alarms.create).toHaveBeenCalledWith(
      "Test-time-slot-restriction-begin",
      { delayInMinutes: 60 }
    );
