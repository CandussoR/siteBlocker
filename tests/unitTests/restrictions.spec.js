...
import { checkTotalTimeRestriction } from '../../worker/blocker/restrictionsHandler.js'
...
describe('totalTime', () => {
    ...
    it('should create an alarm if totalTime is not up', async () => {
        ...
        let result = await checkTotalTimeRestriction(currentDay, 'test2.com', groupRestriction.restrictions.totalTime, false)
    ...
    it('should be restricted if sum of totalTime for group in records is equal or superior to set totalTime', async () => {
        ...
        global.chrome.storage.local.get.mockResolvedValueOnce(fakeSites)
        let result = await checkTotalTimeRestriction(currentDay, 'test.com', groupRestriction.restrictions.totalTime, false)
    ...
    it('should be restricted if site totalTime is set, is lesser than its group totalTime and is already attained', async () => {
        ...
        let result = await checkTotalTimeRestriction(currentDay, 'test.com', groupRestriction.restrictions.totalTime, true, totalTimeRestrictionMock)
        expect(result).toBe(true)
        expect(global.chrome.alarms.create).toHaveBeenCalledTimes(0)
    })
