...
export async function isRestricted(host, sites) {
    ...
    let alarmsEnd = alarms.filter(x => (siteGroup && x.name.includes(siteGroup) && x.name.includes('-end')) || (x.name.includes(host) && x.name.includes('-end')) );
    ...
    let alarmsChecks = alarms.filter(x => (siteGroup && x.name.includes(siteGroup) && x.name.includes('-check')) || (x.name.includes(host) && x.name.includes('-check')));
    ...
    let siteRestrictionTotalTime = findTodayRestriction(currentDay, siteRestrictions, 'totalTime')
    if (siteRestrictionTotalTime) {
        let timeLeft = timeLeftBeforeRestriction(todayRecord[host], siteRestrictionTotalTime, 'totalTime');
        if (timeLeft) {
    ...
    let siteRestrictionConsecutiveTime = findTodayRestriction(currentDay, siteRestrictions, 'consecutiveTime')
    if (siteRestrictionConsecutiveTime) {
      let timeLeft = timeLeftBeforeRestriction(todayRecord[host], siteRestrictionConsecutiveTime, 'consecutiveTime');
        if (timeLeft) {
...
async function isGroupRestricted(host, groupRestrictions, siteRestrictions) {
    ...
    let functionsName = [isGroupRestrictedByTimeSlot, isGroupRestrictedByTotalTime, isGroupRestrictedByConsecutiveTime];
    let i = 0;

    while (!restricted) {
      let todayGroupRestriction = findTodayRestriction(currentDay, groupRestrictions, restrictionsName[i]);
      let todaySiteRestriction = findTodayRestriction(currentDay, siteRestrictions, restrictionsName[i])
      if (!todayGroupRestriction && !todaySiteRestriction) {
        continue;
      }
      restricted = i === 0 
                  ? functionsName[i](todayGroupRestriction, todaySiteRestriction)
                  : await functionsName[i](host, todayGroupRestriction, todaySiteRestriction)
    }
...
export async function isGroupRestrictedByTotalTime(host, groupRestriction, siteRestriction) {
    ...
    if (siteRestriction && siteRestriction !== groupRestriction) {
      siteTimeLeft = timeLeftBeforeRestriction(todayRecord[host], siteRestriction, 'totalTime')
      if (!siteTimeLeft) return true;
    }

    if (siteTimeLeft === -1 || (siteTimeLeft !== -1 && groupTimeLeft <= siteTimeLeft)) {
      console.log("creating alarm with",`${groupName}-total-time-restriction-begin`, groupTimeLeft / 60 )
      await chrome.alarms.create(`${groupName}-total-time-restriction-begin`, {delayInMinutes : groupTimeLeft / 60})
    } else if (groupTimeLeft === -1 || (groupTimeLeft !== -1 && siteTimeLeft < groupTimeLeft)) {
    console.log("creating alarm with",`${host}-total-time-restriction-begin`, siteTimeLeft / 60 )
      await chrome.alarms.create(`${host}-total-time-restriction-begin`, {delayInMinutes : siteTimeLeft / 60})
    } 
...
export async function isGroupRestrictedByConsecutiveTime(host, groupRestriction, siteRestriction) {
  ...
  if (alarms.find(x => x.name === `${groupName}-consecutive-time-restriction-begin` 
  ...
  if (siteRestriction) {
    siteTimeLeft = timeLeftBeforeRestriction(todayRecord[host], siteRestriction, 'consecutiveTime')
    if (!siteTimeLeft) return true;
  }
...
function timeLeftBeforeRestriction(hostRecord, restriction, propertyName) {
  return restriction[propertyName] < hostRecord[propertyName] ? -1 : restriction[propertyName] - hostRecord[propertyName];
}
